...
namespace common {
    ...
    public:
        ...
        bool connect()
        {
            ...
            if (!(csResp[0] == 0 && csResp[1] == TP20ServiceID::ChannelSetupPositiveResponse && encodeLittleEndian(csResp[2], csResp[3]) == requestedChannel)) {
                return false;
            }
            const auto channelTxId{ encodeLittleEndian(csResp[4], csResp[5]) };
    ...
    private:
        bool checkMessageForSkip(const uint8_t* data, size_t dataSize)
        {
            ...
            if (encodeBigEndian(data[0], data[1], data[2], data[3]) != _rxId) {
                return true;
            }
            if (data[4] == 0xA1) {
                return true;
