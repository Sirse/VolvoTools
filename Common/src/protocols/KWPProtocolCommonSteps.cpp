...
namespace common {
    ...
    bool KWPProtocolCommonSteps::authorize(const RequestProcessorBase& requestProcessor, const std::array<uint8_t, 5>& pin)
	{
		try {
			...
			if (seedResponse.size() < 6)
				return false;
            uint32_t key = generateKeyCommon(encodeBigEndian(seedResponse[2], seedResponse[3], seedResponse[4], seedResponse[5]));
                                              ...
                                              const std::function<void(size_t)>& progressCallback)
	{
		try {
			for (const auto& chunk : data.chunks) {
                ...
                const size_t maxSizeToTransfer = encodeBigEndian(downloadResponse[0], downloadResponse[1]) - 2;
	...
	size_t KWPProtocolCommonSteps::requestDownload(const RequestProcessorBase& requestProcessor, const VBFChunk& chunk)
	{
		try {
            ...
            return encodeBigEndian(downloadResponse[0], downloadResponse[1]) - 2;;
		}
		catch (...) {
			return 0;
		}
